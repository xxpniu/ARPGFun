        ...
        public override IEnumerable<RunStatus> Execute(ITreeRoot context)
        {
            ...
            while (true)
            {
                if (!target || target.IsDeath || target.IsLock(ActionLockType.NoInhiden))
                {
                    if (root.IsDebug)
                        Attach("failure",
                            $"target is isDeath:{target.IsDeath} InHidden:{target.IsLock(ActionLockType.NoInhiden)} ");
                ...
                var time = root.Time;
                while (time + .5f > root.Time && root.Character.IsMoving) yield return RunStatus.Running;
