    ...
    public class MagicReleaserControllor : GControllor
    {
        ...
        public override GAction GetAction(GTime time, GObject current)
        {
            ...
            switch (releaser.State)
            {
                ...
                {
                    releaser.OnEvent(EventType.EVENT_START);
                ...
                case ReleaserStates.Starting:
                    ...
                    if (!IsBindAlive(releaser)) releaser.SetState(ReleaserStates.ToComplete);
                ...
                case ReleaserStates.Releasing:
                {
                    if (releaser.Durtime > 0)
                    {
                        ...
                        {
                            releaser.TickTime = 0;
                            releaser.OnEvent(EventType.EVENT_TRIGGER);
                        }
                    ...
                    if (releaser.IsCompleted) releaser.SetState(ReleaserStates.ToComplete);
                ...
                {
                    releaser.OnEvent(EventType.EVENT_END);
