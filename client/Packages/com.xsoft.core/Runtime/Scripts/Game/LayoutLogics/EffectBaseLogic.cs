...
namespace GameLogic.Game.LayoutLogics
{
    ...
    public class EffectBaseLogic
    {
        ...
        private static int GetValueBy(BattleCharacter owner, BattleCharacter target, ValueOf vOf, int value)
        {
            switch (vOf)
            {
                ...
                case ValueOf.NormalAttack:
                    return (int)(BattleAlgorithm.CalFinalDamage(BattleAlgorithm.CalNormalDamage(owner), owner.TDamage,
        ...
        public static void NormalDamage(BattleCharacter effectTarget, EffectBase e, MagicReleaser releaser)
        {
            ...
            var damage = GetValueBy(releaser.Releaser, effectTarget, effect!.valueOf,
                effect.DamageValue.ProcessValue(releaser));
            var result =
                BattleAlgorithm.GetDamageResult(releaser.Releaser, damage, releaser.Releaser.TDamage, effectTarget);
            if (releaser.ReleaserTarget.Releaser.TDamage != DamageType.Magic)
                if (!result.IsMissed)
                {
        ...
        public static void Cure(BattleCharacter effectTarget, EffectBase e, MagicReleaser releaser)
        {
            ...
            if (cure > 0) effectTarget.AddHP(cure);
        }
        ...
        public static void CureMp(BattleCharacter effectTarget, EffectBase e, MagicReleaser releaser)
        {
            ...
            var cure = GetValueBy(releaser.Releaser, effectTarget, effect!.valueType,
                effect.value.ProcessValue(releaser));
        ...
        public static void AddProperty(BattleCharacter effectTarget, EffectBase e, MagicReleaser releaser)
        {
            ...
            if (effect.revertType == RevertType.ReleaserDeath)
                releaser.RevertProperty(effectTarget, effect.property, effect.addType,
                    effect.addValue.ProcessValue(releaser));
        }
        ...
        public static void CharmEffect(BattleCharacter effectTarget, EffectBase e, MagicReleaser releaser)
        {
            ...
            if (!string.IsNullOrEmpty(ai)) per!.ChangeCharacterAI(ai, effectTarget);
