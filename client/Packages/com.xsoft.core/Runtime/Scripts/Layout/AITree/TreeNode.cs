...
namespace Layout.AITree
{
    ...
    public class TreeNode
    {
        ...
        [HideInEditor] public string guid;
        ...
        private static T Initialization<T>(T node) where T : TreeNode, new()
        {
            ...
            var attrs =
                typeof(T).GetCustomAttributes(typeof(EditorAITreeNodeAttribute), false) as EditorAITreeNodeAttribute[];
            if (attrs.Length > 0) node.name = attrs[0].Name;
            return node;
        }

        public static TreeNode CreateInstance(Type t)
        {
            if (!t.IsSubclassOf(typeof(TreeNode))) throw new Exception("[" + t + "] is not subclassof TreeNode");
            ...
            if (attrs.Length > 0) treeNode.name = attrs[0].Name;
            return treeNode;
        }

        public static TreeNodeRelation? FindNodeByGuid(TreeNode root, string guid, TreeNode parent = null)
        {
            if (root.guid == guid) return new TreeNodeRelation { Parant = parent, Node = root };
        ...
        public void NewGuid()
        {
            foreach (var i in childs) i.NewGuid();
        ...
        {
            var attrs =
                GetType().GetCustomAttributes(typeof(EditorAITreeNodeAttribute), false) as EditorAITreeNodeAttribute[];
            if (attrs.Length > 0) return attrs[0].Name;
            return GetType().Name;
        }
