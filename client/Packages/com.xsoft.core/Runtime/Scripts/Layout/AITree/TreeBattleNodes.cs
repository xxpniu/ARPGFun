...
namespace Layout.AITree
{
    ...
    public class TreeNodeBattleEvent : TreeNode
    {
        ...
        [HideInEditor] [Obsolete] public BattleEventType eventType;
    ...
    {
        [Label("距离 cm")] public FieldValue Distance = 0;

        [Label("过滤方式")] public TargetFilterType filter;

        [Label("重新查找")] public bool findNew = true;

        [Label("挑选方式")] public TargetSelectType selectType;

        [Label("阵营类型")] public TargetTeamType teamType;

        [Label("过滤方式使用魔法配置表")] public bool useMagicConfig;

        [Label("距离修正倍数 (万分比)")] public FieldValue ValueMul = 10000;

        [Label("取值来源")] public DistanceValueOf valueOf;

        [Label("视野(0-360)")] public FieldValue View = 360;
    }

    [EditorAITreeNode("等待时间", "Act", "战斗节点", AllowChildType.None)]
    public class TreeNodeWaitForSeconds : TreeNode
    {
        [Label("等待毫秒")] public FieldValue seconds = 1000;
    }
    ...
    {
        [Label("魔法Key")] public string magicKey = string.Empty;

        [Label("目标（获取为目标点或者目标位置）")] public bool ReleaseATPos = false;

        [Label("取值来源")] public MagicValueOf valueOf = MagicValueOf.MagicKey;
    }
    ...
    {
        [Label("比较类型")] public CompareType compareType = CompareType.Less;

        [Label("距离(cm)")] public FieldValue distance = 100;

        [Label("距离修正倍数 (万分比)")] public FieldValue ValueMul = 10000;

        [Label("取值来源")] public DistanceValueOf valueOf = DistanceValueOf.Value;
    }
    ...
    {
        [Label("魔法类型")] public MagicType MTpye = MagicType.MtMagic;

        [Label("魔法选择类型")] public MagicResultType resultType;
    }

    [EditorAITreeNode("移动远离目标", "Act", "战斗节点/移动", AllowChildType.None)]
    public class TreeNodeFarFromTarget : TreeNode
    {
        [Label("远离距离(cm)")] public FieldValue distance = 100;

        [Label("取值方式")] public DistanceValueOf valueOf = DistanceValueOf.Value;
    }

    [EditorAITreeNode("出生点范围内移动", "Act", "战斗节点/移动", AllowChildType.None)]
    public class TreeNodeAroundBornPosMove : TreeNode
    {
        [Label("半径(cm)")] public FieldValue distance = 100;

        [Label("半径取值方式")] public DistanceValueOf Value = DistanceValueOf.Value;
    }

    [EditorAITreeNode("比较目标数", "Cond", "战斗节点/目标", AllowChildType.None)]
    public class TreeNodeCompareTargets : TreeNode
    {
        [Label("比较类型")] public CompareType compareType;

        [Label("比较值(cm)")] public FieldValue compareValue = 0;
        ...
        [Label("距离取值来源")] public DistanceValueOf valueOf = DistanceValueOf.Value;
    }

    [EditorAITreeNode("靠近目标", "Act", "战斗节点/移动", AllowChildType.None)]
    public class TreeNodeMoveCloseTarget : TreeNode
    {
        [Label("停止距离(cm)")] public FieldValue distance = 100;

        [Label("停止取值方式")] public DistanceValueOf valueOf = DistanceValueOf.Value;
    }

    [EditorAITreeNode("随机移动", "Act", "战斗节点/移动", AllowChildType.None)]
    public class TreeNodeMoveRandom : TreeNode
    {
        [Label("移动距离(cm)")] public FieldValue distance = 100;

        [Label("偏移方向")] public FieldValue Forward = 0;
    }
    ...
    {
        //link pos
        [Label("目标坐标")] public Vector3 linkPos = Vector3.zero;
    }
}