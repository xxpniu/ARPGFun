...
public class AITreeEditor : EditorWindow
{
    ...
    //public static GUIStyleState text;
    ...
    static AITreeEditor()
    {
        ...
        foreach (var i in types)
            ...
            {
                var attrs =
                    i.GetCustomAttributes(typeof(EditorAITreeNodeAttribute), false) as EditorAITreeNodeAttribute[];
                if (attrs.Length > 0) _nodeTypes.Add(i, attrs[0]);
            }


        var t = new Color(.3f, .3f, .3f, 1); // Color.white;
        //100,149,237
        _colors.Clear();
        _colors.Add("Seq", new ShortNameColors(1, new Color32(0xCA, 0xE1, 0xFF, 0xFF) * t));
        _colors.Add("PSeq", new ShortNameColors(1, new Color32(0xCA, 0xE1, 0xFF, 0xFF) * t));
        _colors.Add("Sel", new ShortNameColors(1, new Color32(0xEE, 0xD8, 0xAE, 0xFF) * t));
        _colors.Add("PSel", new ShortNameColors(1, new Color32(0xEE, 0xD8, 0xAE, 0xFF) * t));
        _colors.Add("Dec", new ShortNameColors(2, new Color32(0xFF, 0xF6, 0x8F, 0xFF) * t));
        _colors.Add("PRSel", new ShortNameColors(1, new Color32(0xDD, 0xA0, 0xDD, 0xff) * t));
        _colors.Add("PRNode", new ShortNameColors(1, new Color32(0xDD, 0xA0, 0xDD, 0xff) * t));
        _colors.Add("Act", new ShortNameColors(5, new Color32(225, 255, 255, 0xff) * t));
        _colors.Add("Event", new ShortNameColors(3, new Color32(100, 149, 237, 0xFF) * t));
        _colors.Add("Cond", new ShortNameColors(4, new Color32(255, 215, 0, 0xFF) * t));
        _colors.Add("Link", new ShortNameColors(4, new Color32(64, 224, 208, 0xFF) * t));
        _colors.Add("Break", new ShortNameColors(4, new Color32(255, 99, 71, 0xFF) * t));
    }
    ...
    private static readonly Dictionary<string, ShortNameColors> _colors = new();
    private static readonly Dictionary<Type, EditorAITreeNodeAttribute> _nodeTypes = new();
    ...
    private readonly Dictionary<string, StateOfEditor> _expand = new();

    private StateOfEditor this[string key]
    {
        get
        {
            if (_expand.TryGetValue(key, out var e)) return e;
    ...
    [NonSerialized] private TreeNode root;
    ...
    public void OnGUI()
    {
        ...
        GUI.Label(new Rect(position.width - 300, 0, 150, 20), text, guiSty);

        //Debug.Log(title);

        if (GUI.Button(new Rect(position.width - 50, 0, 45, 20), "Reset"))
        {
            scale = 1;
            offsetPos = Vector2.zero;
        }

        if (!string.IsNullOrEmpty(currenPath)) GUI.Label(new Rect(3, 0, position.xMax - 10, 20), currenPath);
        GUI.EndGroup();

        GUI.BeginGroup(new Rect(0, 20, 100000, 100000));
        var m = GUI.matrix;
        var sm = m *
                 Matrix4x4.TRS(new Vector3(offsetPos.x, offsetPos.y, 0),
        ...
        if (currentDrag == null && Event.current.type == EventType.MouseDown) _beginDraging = true;
        //Event.current.Use();
    }

    private bool _beginDraging;
    ...
    private void OnDrawGUI()
    {
        if (root == null)
        {
            ...
            if (!string.IsNullOrEmpty(currenPath)) OpenTree(currenPath);
        }
        else
        {
            runstate = null;
            ...
            // RunStatus? runState;
            var runing = CheckRunning(root.guid, out var runState);
            //GUI.BeginClip (rect);
            //lastoffset =
            DrawRoot(root, new Vector2(offsetx, offsety), runing, runState, out var mine);
            //GUI.EndClip ();
            //lastoffset.x;
            ...
            if (Event.current.type == EventType.MouseDrag) currentShowDrag = null;

            if (currentShowDrag.HasValue) GLDraw.DrawFillBox(currentShowDrag.Value, Color.black, Color.green, 1);
            //GUI.EndScrollView();
        }
    }
    ...
    private void DrawOp()
    {
...

        {
            ...
            if (GUILayout.Button("Run", GUILayout.Width(50))) RunAI();
            ...
            if (GUILayout.Button("Save As", GUILayout.Width(50))) SaveAs();
    ...
    {
        var m = userState as MenuState;
        var path = EditorUtility.OpenFilePanel("Import Ai Tree", $"{Application.dataPath}{AI_ROOT}", "xml");
    ...
    private void SaveAs()
    {
        currenPath = EditorUtility.SaveFilePanel("Save Tree", $"{Application.dataPath}{AI_ROOT}", "AITree_" + root.guid,
            "xml");
        if (string.IsNullOrEmpty(currenPath))
            return;
    ...
    private bool CheckRunning(string guid, out RunStatus? state)
    {
        ...
        return comp.LastStatus.Value == RunStatus.Running;
        //return false;
    }
    ...
    private Vector2 DrawRoot(TreeNode node, Vector2 offset, bool isRuning, RunStatus? state, out Vector2 current)
    {
        ...
        var offex = tempOffset.x;
        var expand = this[node.guid];
        var points = new List<LineData>();
        if (expand.Expanded)
            for (var i = 0; i < node.childs.Count; i++)
            {
                if (node.childs[i] == currentDrag)
                    continue;
                var runing = CheckRunning(node.childs[i].guid, out var cRunstate);
                //var mine = tempOffset;
                var or = DrawRoot(node.childs[i], tempOffset, runing, cRunstate, out var mine);
    ...
    private StateOfEditor DrawNode(Rect rect, TreeNode node, StateOfEditor expanded, bool haveChild, bool isRuning,
        RunStatus? state)
    {
        var color = isRuning ? Color.yellow : Color.black;


        var bg = Color.white;
        var att = GetNodeAtt(node.GetType());

        if (lastClick == node) color = Color.green;
        var name = node.GetType().Name;
        if (att != null)
        {
            var str = state == null ? "" : $"[{state.Value.ToString()}]";
            name = $"{str}{att.ShorName}:{node.name}";
    ...
    {
        var hy = HoverType.Middle;
        var p = rect.height / 3;

        if (y < p + rect.y)
            hy = HoverType.Top;
        else if (y > 2 * p + rect.y) hy = HoverType.Bottom;
        return hy;
    }
        ...
        if (reDrag != null && hove != null)
            switch (type)
            {
                case HoverType.Bottom:
                {
                    ...
                    {
                        var index = hove.Value.Parant.childs.IndexOf(hoverNode) + 1;
                        if (reDrag.Value.Parant == hove.Value.Parant)
                        {
                            var a = reDrag.Value.Parant.childs.IndexOf(currentDrag);
                ...
                case HoverType.Top:
                {
                    if (CanAppend(hove.Value.Parant, currentDrag))
                    {
                        var index = hove.Value.Parant.childs.IndexOf(hoverNode);
