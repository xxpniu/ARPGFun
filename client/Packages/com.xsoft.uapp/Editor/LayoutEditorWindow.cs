...
public class LayoutEditorWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (currentObj == line)
            if (Event.current.type == EventType.MouseDrag)
                if (rectTop.Contains(Event.current.mousePosition))
                {
                    ...
                    {
                        if (currentzTime < _s && Math.Abs(_lastStep) > 0.000001) PlayLayout();
                        _s = Time.deltaTime;
        ...
        if (Event.current.type == EventType.ContextClick)
            if (rectTop.Contains(Event.current.mousePosition))
            {
                ...
                m.AddSeparator("");
                foreach (var i in Layouts) m.AddItem(new GUIContent(i.Value), false, CreateLayout, i.Key);
                m.ShowAsContext();
        ...
        for (var i = 0; i < line.Layouts.Count; i++)
        {
            ...
            var attrs =
                l.GetType().GetCustomAttributes(typeof(EditorLayoutAttribute), false) as EditorLayoutAttribute[];
            if (attrs.Length > 0)
                name = attrs[0].Name;
            ...
            foreach (var point in points)
            {
                var time = point.Time;
                var withd = 1;
                var xt = rect.width * (time / line.Time);
            ...
            if (currentObj != l)
                if (Event.current.type == EventType.MouseDown)
                    if (rect.Contains(Event.current.mousePosition))
                    {
                        if (currentObj != l) ShowProperty(l); //	currentObj ;
                        Event.current.Use();
                    }
        ...
        if (currentObj == line)
            GLDraw.DrawBox(
                new Rect(rectTop.x + offset, rectTop.y + offset, rectTop.width - offset * 2,
                    rectTop.height - offset * 2),
                Color.green, 2);
        //show Lines
        ...
        for (var i = 0; i <= count; i++)
        {
            ...
            if (h == rectTop.height)
                GUI.Label(new Rect(x, 16, 20, 20), string.Format("{0:0.0}", line.Time * (i / (float)count)));
    ...
    {
        var window = (LayoutEditorWindow)GetWindow(typeof(LayoutEditorWindow), false, "Layout Editor");
    ...
    {
        Init();
        var window = GetWindow<LayoutEditorWindow>();
    ...
    private void PlayLayout()
    {
        ...
        var testMagic = new MagicData();
        testMagic.Containers.Add(new EventContainer
            {
    ...
    private void GetPlayingInfo()
    {
        ...
        CurrentRunTime = EditorStarter.Try()?.currentReleaser?.GetLayoutTimeByPath(shortPath);
    }
    ...
    {
        var layoutBase = userstate as LayoutBase;
        line.Points.Add(new TimePoint { Time = currentzTime, GUID = layoutBase.GUID });
    }
    ...
    {
        if (currentObj != obj) GUI.UnfocusWindow();
        currentObj = obj;
    }
