...
public class MagicEditorWindow : EditorWindow
{
    ...
    [NonSerialized] private MagicData data;

    private Vector2 scrollProperty;
    private Rect view = new(0, 0, 1000, 1000);
    ...
    private void OnGUI()
    {
        ...
        float lS = 230;
        DrawOp();
        GUI.Label(new Rect(0, 0, position.width, 30), currentPath);
        ...
        var cOffset = new Vector2(offsetX + sizeX + offsetX, offsetY);

        var eOffset = new Vector2(cOffset.x + sizeX + offsetX, offsetY);
        float maxY = 0;
        var indexC = 100;
        var cEndPoint = new List<TargetPoint>();
        foreach (var i in data.Containers)
        {
            var indexE = indexC * 100;
            ;
            ...
            foreach (var e in i.effectGroup)
            {
                ...
                if (DrawWindow(indexE, eRect, EffectWindow, e.key)) ShowObject(e);
    ...
    {
        var window = (MagicEditorWindow)GetWindow(typeof(MagicEditorWindow), false, "Magic Editor");
    ...
    private bool IsRuning(EventType type)
    {
        if (!EditorApplication.isPlaying) return false;
        return AIRunner.Current?.IsRunning(type) ?? false;
    }
    ...
    private void DrawOp()
    {
        ...
        if (GUILayout.Button("New", GUILayout.Width(50))) New();
        if (GUILayout.Button("Open", GUILayout.Width(50))) Open();

        if (GUILayout.Button("Save", GUILayout.Width(50))) Save(data);

        if (GUILayout.Button("Save As", GUILayout.Width(50))) SaveAs(data);
    ...
    {
        if (data != null)
            if (!EditorUtility.DisplayDialog("Cancel", "The operator will lost current edit,Do you want to over it!",
                    "Yes", "Cancel"))
                return;

        currentPath = null;
        data = new MagicData { name = "New Magic" };
        data.Containers.Add(new EventContainer { type = EventType.EVENT_START });
    }

    private void Open()
    {
        if (data != null)
            if (!EditorUtility.DisplayDialog("Cancel", "Open file will lost current edit，Do you want to over it", "Yes",
                    "Cancel"))
                return;
    ...
    {
        if (!(userstate is MagicData data)) return;
        data.Containers.Add(new EventContainer { type = EventType.EVENT_TRIGGER });
    }
    ...
    private bool DrawWindow(int id, Rect r, GUI.WindowFunction fun, string name)
    {
        ...
        if (Event.current.type == UnityEngine.EventType.MouseDown &&
            r.Contains(Event.current.mousePosition)) return true;
        return false;
    }

    private void EffectWindow(int id)
    {
        var indexC = id / 100 - 100;
        var con = data.Containers[indexC];
        var indexE = id - id / 100 * 100;
