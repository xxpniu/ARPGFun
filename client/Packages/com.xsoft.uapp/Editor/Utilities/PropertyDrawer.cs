...
public static class PropertyDrawer
{
    ...
    private static void Init()
    {
        ...
        foreach (var i in att) tnames.Add(i.Name);
    ...
    public static void DrawObject(object obj, string key)
    {
        ...
        {
            if (_dic.TryGetValue(key, out var last))
                _dic[key] = obj;
            else
                _dic.Add(key, obj);
    ...
    {
        if (info is FieldInfo field) return field.GetValue(obj);
        if (info is PropertyInfo property) return property.GetValue(obj);
        return null;
    }

    private static void SetMemberValue(this MemberInfo info, object obj, object value)
    {
        if (info is FieldInfo field) field.SetValue(obj, value);
        if (info is PropertyInfo property) property.SetValue(obj, value);
    }

    private static Type GetFieldType(this MemberInfo info)
    {
        if (info is FieldInfo field) return field.FieldType;
        if (info is PropertyInfo property) return property.PropertyType;
        return null;
    }
    ...
    private static void DrawProperty(MemberInfo field, object obj)
    {
        ...
        foreach (var i in _handlers)
        {
            ...
            if (attrs.Length > 0)
            {
                i.Value.Invoke(null, new[] { obj, field, name, attrs[0] });
                return;
            }
        }
        ...
        {
            //GUILayout.Label (name);
            var v = (Vector3)field.GetMemberValue(obj);
            var value = EditorGUILayout.Vector3Field(name, new UnityEngine.Vector3(v.x, v.y, v.z));
            field.SetMemberValue(obj, new Vector3 { x = value.x, y = value.y, z = value.z });
        }
        else if (fType == typeof(FieldValue))
        {
            GUILayout.Label(name);
            if (!(field.GetMemberValue(obj) is FieldValue value)) value = 1000;
        ...
        else if (fType == typeof(DamageRange))
        {
            ...
            value.fiterType = (FilterType)EditorGUILayout.EnumPopup(value.fiterType);
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Label(GetLable(typeof(DamageRange).GetField("damageType")));
            value.damageType = (DamageType)EditorGUILayout.EnumPopup(value.damageType);
            GUILayout.EndHorizontal();
            if (value.damageType == DamageType.Area)
            {
                var fieldNames = new[] { "radius", "angle", "offsetAngle", "offsetPosition" };
                foreach (var i in fieldNames) DrawProperty(typeof(DamageRange).GetField(i), value);
            }
        ...
        else if (fType == typeof(ValueSourceOf))
        {
            ...
            value.ValueForm = (GetValueFrom)EditorGUILayout.EnumPopup(value.ValueForm);
            if (value.ValueForm == GetValueFrom.CurrentConfig) value.Value = EditorGUILayout.IntField(value.Value);
            GUILayout.EndHorizontal();
    ...
    public static void LayoutPathSelect(object obj, MemberInfo field, string label, object attr)
    {
        ...
        {
            var fPath = EditorUtility.SaveFilePanel("Create Layout", $"{Application.dataPath}{ASSET_ROOT}Layouts/",
                "layout", "xml");
    ...
    public static void BoneSelected(object obj, MemberInfo field, string label, object attr)
    {
        ...
        if (indexOfBone >= 0 && indexOfBone < names.Length) field.SetMemberValue(obj, names[indexOfBone]);
    }
}