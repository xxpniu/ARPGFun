...
public class GLDraw
{
    ...
    public static void DrawLine(Vector2 start, Vector2 end, Color color, float width)
    {
        ...
        else
        {
            ...
            var perpendicular = (startPt - endPt).normalized * width;
            var v1 = new Vector3(start.x, start.y, 0);
            var v2 = new Vector3(end.x, end.y, 0);
    ...
    public static void DrawCircle(Vector2 pos, float r, Color color, float width)
    {
        ...
        GL.Color(color);
        for (var i = 0; i < n; ++i)
            GL.Vertex(new Vector3(r * Mathf.Cos(2 * Mathf.PI / n * i) + pos.x,
                r * Mathf.Sin(2 * Mathf.PI / n * i) + pos.y, 0));
        GL.End();
    }
    ...
    public static void DrawRect(Rect box, Color color)
    {
        ...
        var p1 = new Vector2(box.xMin, box.yMin);
        var p2 = new Vector2(box.xMax, box.yMin);
        var p3 = new Vector2(box.xMax, box.yMax);
        var p4 = new Vector2(box.xMin, box.yMax);
    ...
    {
        var box = new Rect(topLeftCorner.x, topLeftCorner.y, bottomRightCorner.x - topLeftCorner.x,
    ...
    public static void DrawConnectingCurve(Vector2 start, Vector2 end, Color color, float width)
    {
        ...
        var segments = Mathf.FloorToInt(distance.magnitude / 20 * 3);
    ...
    public static void DrawBezier(Vector2 start, Vector2 startTangent, Vector2 end, Vector2 endTangent, Color color,
        float width)
    {
        var segments = Mathf.FloorToInt((start - end).magnitude / 20) * 3; // Three segments per distance of 20
    ...
    {
        var startVector = CubeBezier(start, startTangent, end, endTangent, 0);
        for (var i = 1; i <= segments; i++)
        {
            var endVector = CubeBezier(start, startTangent, end, endTangent, i / (float)segments);
