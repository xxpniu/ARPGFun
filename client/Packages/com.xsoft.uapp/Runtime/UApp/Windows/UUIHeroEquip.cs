...
namespace Windows
{
    internal partial class UUIHeroEquip
    {
        ...
        protected override void InitModel()
        {
            ...
            foreach (var i in Equips) i.Value.bt.onClick.AddListener(() => { Click(i.Key); });
            ...
            async void TakeOffCall()
            {
                ...
                var g = GateManager.S;
                if (!g.Package.Items.TryGetValue(_selected.GUID, out var item)) return;
        ...
        private async void DisplayEquip(WearEquip eq)
        {
            ...
            var g = GateManager.S;
            g.Package.Items.TryGetValue(eq.GUID, out var it);
            ...
            var level = ExcelToJSONConfigManager.First<EquipmentLevelUpData>(t =>
                t.Level == it.Level && t.Quality == item.Quality);
            var next = ExcelToJSONConfigManager.First<EquipmentLevelUpData>(t =>
                t.Level == it.Level + 1 && t.Quality == item.Quality);
        ...
        {
            Level.text = LanguageManager.S.Format("UUIHeroEquip_level", $"{dHero.Level}");
            ...
            foreach (var i in statData) properties.Add((P)i.ID, i.InitValue);
            ...
            {
                if (!package.Items.TryGetValue(i.GUID, out var pItem)) continue;
            ...
            {
                var stat = ExcelToJSONConfigManager.GetId<StatData>((int)i.Key);
                PropertyListTableManager[index].Model
                    .SetLabel(LanguageManager.S[stat.WordKey], $"{i.Value.ToValueString(i.Key)}");
                index++;
            }
        }
        ...
        public class PropertyListTableModel : TableItemModel<PropertyListTableTemplate>
        {
            ...
            {
                Template.lb_text.text = $"{key}:{value}";
            }
        }
