...
namespace UApp
{
    ...
    public class GateManager : XSingleton<GateManager>
    {
        ...
        {
            coin = coinAndGold.Coin;
            gold = coinAndGold.Gold;
        }
        ...
        {
            if (_login is { HavePlayer: false }) await Release();
            ...
            Call = _client.CreateClient<ServerStreamService.ServerStreamServiceClient>()
                .ServerAnyStream(new Void(),
                    cancellationToken: _client.ShutdownToken);

            HandleChannel = new Stream.ResponseChannel<Any>(Call.ResponseStream, true, "MainGateHandle")
            {
        ...
        public async void SendCommand(string command)
        {
            ...
            var r = await S.GateFunction.GMToolAsync(new C2G_GMTool
            {
                GMCommand = command
            });
