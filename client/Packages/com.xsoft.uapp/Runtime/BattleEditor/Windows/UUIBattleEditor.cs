...
namespace Windows
{
    ...
    internal partial class UUIBattleEditor
    {
        protected override void InitModel()
        {
            ...
            bt_releaser.onClick.AddListener(() =>
            {
                ...
                if (!int.TryParse(input_index.text, out var charId)) return;
            ...
            bt_targe.onClick.AddListener(() =>
            {
                int.TryParse(input_Level.text, out var level);
                level = Mathf.Clamp(level, 1, 100);
                if (int.TryParse(input_index.text, out var charId))
                {
                    var character = ExcelToJSONConfigManager.GetId<CharacterData>(charId);
                    if (character == null) return;
            ...
            bt_normal_att.onClick.AddListener(() => { EditorStarter.S.DoAction(new Action_NormalAttack()); });
            //Write Code here
        }
