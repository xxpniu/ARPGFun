...
namespace BattleEditor
{
    public class EditorStarter : XSingleton<EditorStarter>, IAIRunner, IStateLoader
    {
        ...
        {
            if (!stay && releaser) releaser.SubHP(releaser.HP, out _);
            var per = _curState.Perception as BattlePerception;
        ...
        public void ReplaceTarget(int level, CharacterData data, bool stay, bool ai)
        {
            ...
            var character = per!.CreateCharacter(per.AIControllor, level, data, magics, properties, 2,
                scene.enemyStartPoint.position + UVector3.left * distanceCharacter / 2,
                new UVector3(0, -90, 0), string.Empty, data.Name);
            //target.ResetHPMP();
        ...
        public void DoAction(IMessage action)
        {
            ...
            if (releaser.AiRoot == null) return;
            releaser?.AddNetAction(action);
        }
        ...
        private async void Start()
        {
            ...
            await UUIManager.S.CreateWindowAsync<UUIBattleEditor>(ui => ui.ShowWindow());
        ...
        private void Tick()
        {
            if (_curState != null && _isStarted) GState.Tick(_curState, Now);
        }
