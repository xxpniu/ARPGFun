...
namespace BattleViews.Views
{
    public class UPerceptionView : MonoBehaviour, IBattlePerception, ITimeSimulator, IViewBase
    {
        ...
        private readonly Dictionary<int, UElementView> _attachElements = new();

        private readonly IMessage[] _empty = Array.Empty<IMessage>();

        private readonly Queue<IMessage> _notify = new();

        private readonly Dictionary<int, UMagicReleaserView> _ownerReleasers = new();
        ...
        {
            if (_attachElements.TryGetValue(releaseIndex, out var vi))
                return vi as T;
            return null;
        }
        ...
        public void DeAttachView(UElementView battleElement)
        {
            ...
            if (r.CharacterReleaser.Index == OwnerIndex) _ownerReleasers.Remove(r.Index); //, r);
        }
        ...
        private TimeLine TryToLoad(string path)
        {
            ...
            if (UseCache) _timeLines.Add(path, line);
            return line;
        }
        ...
        bool IBattlePerception.ProcessDamage(int owner, int target, int damage, bool isMissed, int crtmult)
        {
            ...
            if (crtmult > 1)
                GPUBillboardBuffer.S.DisplayNumberRandom(num,
        ...
        TimeLine IBattlePerception.GetTimeLineByPath(string path)
        {
            ...
            line = TryToLoad(path);
            if (line == null) Debug.LogError($"Not found:{path}");
            return line;
        }
        ...
        IBattleCharacter IBattlePerception.CreateBattleCharacterView(string accountId,
            int config, int teamId, Vector3 pos, Vector3 forward,
            int level, string characterName, IList<HeroMagicData> cds, int owner, IList<HeroProperty> properties,
            int hp, int mp)
        {
            var dic = new Dictionary<HeroPropertyType, ComplexValue>();
            foreach (var i in properties) dic.Add(i.Property, i.Value);
            ...
            if (cds != null)
                foreach (var i in cds)
                    view.AddMagicCd(i.MagicID, i.CDCompletedTime, i.MType, i.CdTotalTime, i.MPCost);
            if (view is IBattleCharacter ch)
                ch.SetHpMp(hp, dic[HeroPropertyType.MaxHp], mp, dic[HeroPropertyType.MaxMp]);
        ...
        IBattleMissile IBattlePerception.CreateMissile(int releaseIndex, int targetIndex, string res, Vector3 offset,
            string fromBone, string toBone, float speed, int mType, float maxDis, float maxLifeTime)
        {
        ...
        ITimeSimulator IBattlePerception.GetTimeSimulator()
        {
            return this;
        }
        ...
        IBattlePerception IViewBase.Create(ITimeSimulator simulator)
        {
            return this;
        }
        ...
        IBattleItem IBattlePerception.CreateDropItem(Vector3 pos, PlayerItem item, int teamIndex, int groupId)
        {
