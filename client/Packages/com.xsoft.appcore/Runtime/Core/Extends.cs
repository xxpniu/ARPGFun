...
namespace App.Core.Core
{
    public static class Extends
    {
        ...
        private static readonly JsonFormatter format = new(JsonFormatter.Settings.Default
            .WithFormatEnumsAsIntegers(true).WithFormatDefaultValues(true));
            ...
            for (var i = 0; i < raw; i++)
            {
                ...
                for (var index = 0; index < rawData.Count; index++)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Debug.LogError(
                            $"{typeof(T)} {property.Info.PropertyType} {property.Info.Name} [{index}] of {rawData[index]} - {rawData}");
                    }
                }
