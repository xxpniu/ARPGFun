...
namespace App.Core.Core
{
    ...
    public class ResourcesManager : XSingleton<ResourcesManager>, IConfigLoader
    {
        ...
        public async Task<T> LoadResourcesWithExName<T>(string path, CallBackDele<T> call = null,
        ...
        public async Task<Sprite> LoadIcon(ItemData item, CallBackDele<Sprite> callBack = null,
            CancellationToken? token = default)
        {
            return await LoadSpriteAsset($"Icon/{item.Icon}.png", callBack, token);
        }

        public async Task<Sprite> LoadIcon(BattleLevelData item, CallBackDele<Sprite> callBack = null,
            CancellationToken? token = default)
        {
        ...
        public async Task<GameObject> LoadModel(ItemData item, CallBackDele<GameObject> call = null,
            CancellationToken? token = default)
        {
